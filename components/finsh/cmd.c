...
static long _list_thread(struct rt_list_node *list) {
  ...
  for (node = list->next; node != list; node = node->next) {
    ...
    rt_kprintf(" 0x%08x 0x%08x    %02d%%   0x%08x %03d\n",
               thread->stack_size +
                   ((rt_uint32_t)thread->stack_addr - (rt_uint32_t)thread->sp),
               thread->stack_size,
               (thread->stack_size -
                ((rt_uint32_t)ptr - (rt_uint32_t)thread->stack_addr)) *
...
static long _list_mailbox(struct rt_list_node *list) {
  ...
  maxlen = object_name_maxlen(list);
  if (maxlen < item_title_len)
    maxlen = item_title_len;
...
static long _list_msgqueue(struct rt_list_node *list) {
  ...
  for (node = list->next; node != list; node = node->next) {
    ...
    if (!rt_list_isempty(&m->parent.suspend_thread)) {
      rt_kprintf("%-*.*s %04d  %d:", maxlen, RT_NAME_MAX, m->parent.parent.name,
  ...
  for (node = list->next; node != list; node = node->next) {
    ...
    if (mp->suspend_thread_count > 0) {
      rt_kprintf("%-*.*s %04d  %04d  %04d %d:", maxlen, RT_NAME_MAX,
                 ...
                 mp->block_free_count, mp->suspend_thread_count);
...
static long _list_timer(struct rt_list_node *list) {
  ...
  for (node = list->next; node != list; node = node->next) {
    ...
    rt_kprintf("%-*.*s 0x%08x 0x%08x ", maxlen, RT_NAME_MAX, timer->parent.name,
               timer->init_tick, timer->timeout_tick);
...
static long _list_device(struct rt_list_node *list) {
  ...
  struct rt_list_node *node;
  char *const device_type_str[] = {"Character Device",
                                   "Block Device",
                                   "Network Interface",
                                   "MTD Device",
                                   "CAN Device",
  ...
  for (node = list->next; node != list; node = node->next) {
    ...
    rt_kprintf("%-*.*s %-20s %-8d\n", maxlen, RT_NAME_MAX, device->parent.name,
  ...
  if ((module = rt_module_find(name)) != RT_NULL) {
    ...
    if (!(module->parent.flag & RT_MODULE_FLAG_WITHOUTENTRY)) {
      ...
      if (module->module_thread != RT_NULL) {
        ...
        rt_kprintf("main thread  pri  status      sp     stack size max used   "
                   "left tick  error\n");
        rt_kprintf("------------- ---- ------- ---------- ---------- "
                   "---------- ---------- ---\n");
        ...
        rt_kprintf(" 0x%08x 0x%08x 0x%08x 0x%08x %03d\n",
                   thread->stack_size + ((rt_uint32_t)thread->stack_addr -
...
long list(void) {
  ...
  rt_kprintf("--Variable List:\n");
  {
    ...
    for (index = _sysvar_table_begin; index < _sysvar_table_end;
