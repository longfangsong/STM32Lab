...
void delay_us(rt_uint16_t time) {
    u16 i = 0;
    while (time--) {
        i = 10;
        while (i--);
...
// color:颜色
// num:数值(0~4294967295);
void LCD_ShowNum(u16 x, u16 y, rt_uint32_t num, u8 len, u8 size) {
    u8 t, temp;
    u8 enshow = 0;
    for (t = 0; t < len; t++) {
...
//[6:1]:保留
//[0]:0,非叠加显示;1,叠加显示.
void LCD_ShowxNum(u16 x, u16 y, rt_uint32_t num, u8 len, u8 size, u8 mode) {
    u8 t, temp;
    u8 enshow = 0;
    for (t = 0; t < len; t++) {
        temp = (num / LCD_Pow(10, len - t - 1)) % 10;
