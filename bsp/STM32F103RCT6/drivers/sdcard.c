...
void SD_SPI_Init(void) {
  ...
  GPIO_InitStructure.GPIO_Pin =
      GPIO_Pin_2 | GPIO_Pin_3 | GPIO_Pin_4; // PA2.3.4 推挽 	n_3|GPIO_Pin_4;
...
static rt_err_t rt_sdcard_close(rt_device_t dev) { return RT_EOK; }

static rt_size_t rt_sdcard_read(rt_device_t dev, rt_off_t pos, void *buffer,
                                rt_size_t size) {
  SD_Error status;
  rt_uint32_t retry;
  rt_uint32_t factor;
  ...
  while (retry) {
    /* read all sectors */
    if (((rt_uint32_t)buffer % 4 != 0) || ((rt_uint32_t)buffer > 0x20080000)) {
      rt_uint32_t index;
      ...
      for (index = 0; index < size; index++) {
        ...
        /* copy to the buffer */
        rt_memcpy(((rt_uint8_t *)buffer + index * SECTOR_SIZE), _sdcard_buffer,
                  SECTOR_SIZE);
      }
    } else {
      if (size == 1) {
        status = SD_ReadBlock((part.offset + pos) * factor, (uint32_t *)buffer,
                              SECTOR_SIZE);
      } else {
...
static rt_size_t rt_sdcard_write(rt_device_t dev, rt_off_t pos,
  ...
  if (((rt_uint32_t)buffer % 4 != 0) || ((rt_uint32_t)buffer > 0x20080000)) {
    ...
    for (index = 0; index < size; index++) {
      /* copy to the buffer */
      rt_memcpy(_sdcard_buffer, ((rt_uint8_t *)buffer + index * SECTOR_SIZE),
                SECTOR_SIZE);
    ...
    if (size == 1) {
      status = SD_WriteBlock((part.offset + pos) * factor, (uint32_t *)buffer,
                             SECTOR_SIZE);
    } else {
